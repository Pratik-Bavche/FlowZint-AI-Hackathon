        ...
        <div className="w-full max-w-md bg-[#111] border border-white/10 rounded-2xl p-8 shadow-2xl relative">
          ...
          <button 
            ...
            onClick={() => {
              ...
              setTimeout(() => {
                localStorage.setItem('isAuthenticated', 'true');
                localStorage.setItem('userEmail', 'alex.demo@gmail.com');
                localStorage.setItem('token', 'mock-google-oauth-token');
                window.location.href = '/dashboard';
              }, 1200);
            }}
            disabled={isLoading}
            className="mt-6 w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 hover:bg-white/10 text-white rounded-xl py-2.5 font-medium text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            ...
            {isLoading ? 'Connecting to Google...' : 'Google'}
          </button>
        </div>
      </main>
    </div>
